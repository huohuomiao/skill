...
EnvConfig 产出人类可读的环境记录、设备信息原文和稳定的运行上下文；下游需要运行代码时，继续
...
| 文件输出 | `{输出存储路径}/EnvConfig/run_context.json` - 缓存指纹和回归可比性使用的结构化上下文 |
...
根据**真实探测输出**生成 `{输出存储路径}/EnvConfig/run_context.json`：

```json
{
  "schema_version": 1,
  "execution_backend": "local 或 worker",
  "hardware_key": "<设备型号:集群数:显存等足以区分性能环境的稳定标识>",
  "toolchain_key": "<NeuWare:Triton:PyTorch 等版本的稳定标识>",
  "device_info": {},
  "toolchain_info": {}
}
```

- `hardware_key` 和 `toolchain_key` 必须由探测输出归一化，不得猜测；字段不足时 EnvConfig 失败，禁止启用运行态缓存或性能回归比较。
- key 中不得包含时间戳、task_id、临时目录等每次运行都会变化的内容。
- `device_info`、`toolchain_info` 保存构成 key 的原始字段，便于审计。
- Worker 模式使用 Worker 的设备和工具链信息，不能写本机 CPU 环境信息。
...
```json
{
  ...
  "run_context_path": "{输出存储路径}/EnvConfig/run_context.json"
}
```
...
| 运行上下文 | 校验 `run_context.json` | backend、hardware_key、toolchain_key 均来自真实探测且非空 |
...
| 脚本 | 位置 | 功能 |
|------|------|------|
| `test_env_code.py` | `.claude/skills/share/mlu/runtime/test_env_code.py` | Triton 环境自检脚本，可直接在本地执行；Worker 模式下作为 `submit_task_to_worker.py --command` 的参数执行 |
| `get_device_info.py` | `.claude/skills/share/mlu/runtime/get_device_info.py` | MLU 设备信息采集脚本，可直接在本地执行；Worker 模式下作为 `submit_task_to_worker.py --command` 的参数执行 |
