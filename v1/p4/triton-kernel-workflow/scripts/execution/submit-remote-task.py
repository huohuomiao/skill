...
This is the only script Triton skills should use for real compile/run/accuracy/
performance work. It posts one Task through Agent-Service, polls until
...
退出码：
    ...
    3  Task canceled
    4  p3 tuning budget exhausted before submission
"""
import argparse
import json
import os
...
def _err(msg):
    print(f"[submit_remote_task][error] {msg}", file=sys.stderr)


def _info(msg):
    print(f"[submit_remote_task] {msg}", file=sys.stderr)
...
def _parse_args():
    ...
    p.add_argument(
        "--budget-state",
        type=Path,
        help="p3 tuning_state.json; reserves one Worker call before submission",
    )
    p.add_argument(
        "--budget-label",
        default="worker-task",
        help="event label written when --budget-state is supplied",
    )
    return p.parse_args()


def _reserve_budget(args):
    ...
    if result.returncode == 3:
        _info(f"tuning budget exhausted: {result.stdout.strip()}")
        sys.exit(4)
    if result.returncode != 0:
        _err(f"cannot reserve tuning budget: {result.stderr.strip()}")
        sys.exit(2)
    try:
        state = json.loads(result.stdout)
        remaining = int(state["remaining_seconds"])
    except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
        _err(f"invalid tuning budget response: {exc}")
        sys.exit(2)
    if remaining <= 0:
        _info("tuning budget exhausted before submission")
        sys.exit(4)
    args.timeout_sec = min(args.timeout_sec, remaining)
    return time.monotonic() + remaining


def _stop_budget(state_path, reason):
...
def _poll_until_terminal(task_id, poll_interval, timeout_sec, expires_at, budget_deadline=None):
    ...
    if budget_deadline is not None:
        ...
        deadline = min(deadline, time.time() + remaining)
...
def main():
    ...
    budget_deadline = _reserve_budget(args)
    ...
    status = _poll_until_terminal(
        task_id, args.poll_interval, timeout_sec, expires_at, budget_deadline
    )

    if status == "budget-exhausted":
        _stop_budget(args.budget_state, "elapsed-limit")
        _err(f"tuning wall-time budget expired while waiting for task_id={task_id}")
        sys.exit(4)
