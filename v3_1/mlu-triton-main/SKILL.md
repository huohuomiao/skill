...
**恢复规则（必须执行）**：首次进入且 plan/state 都不存在时调用普通 `plan`；重试或中断恢复时，若两个文件都存在，必须调用 `plan --resume`。只有一个文件或恢复校验不兼容时停止并报告，禁止删除、覆盖或重建状态。`--resume` 成功后从 `optimization_state.json` 的累计 usage、best、patience 和 history 继续。
