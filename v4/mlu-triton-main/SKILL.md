...
- 任何 `complete` / `cached` 记录只有在当前 Skill 源指纹和产物 SHA-256 都有效时才能复用；失效从本阶段向全部下游传播。
...
先执行 `next`：若返回 `extractor/restore`，执行 `restore --stage extractor` 并跳过 Subagent；若返回 `extractor/run`，执行 `start --stage extractor` 后再分发任务。成功后执行 `complete --stage extractor`，失败则记录 `fail`。
...
**阶段控制（必须执行）**：调用 Skill 前服从 `next` 对 `kernel_gen` 的判定。`restore` 命中时由控制器恢复并校验 `triton_code_fix.py`、`triton_report.md`，不得再次调用 Code Gen；`run` 时先 `start`。交接验证通过后 `complete --stage kernel_gen`，失败时记录 `fail`。未绑定 `run_context.json` 时不得开始本阶段。
...
**阶段控制（必须执行）**：`correctness` 下控制器已把 `optimizer` 标为 `skipped`。其他模式先服从 `next`：`restore` 时恢复四个 Optimizer 交接文件并跳过 Skill；`run` 时先 `start --stage optimizer`。若目录中已有计划和状态，必须把“恢复执行”传给 Optimize，使其使用 `plan --resume`；若只有一个文件或输入/模式/预算不兼容，停止并报告，禁止重建状态。成功后 `complete --stage optimizer`，失败时 `fail`。
...
- 规范化回归结果：`{output_dir}/regression_result.json`（符合 `validation/contracts/regression_result.schema.json`，供 Nightly/Release 门禁直接读取）
...
{output_dir}/
├── run_manifest.json               # v4：阶段状态、指纹、产物哈希与恢复审计
├── EnvConfig/                      # 步骤 1：环境配置
