...
def _load_impact(root: Path, path: Path | None) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    if path is None:
        return None, make_check("change-impact-plan", "pass", scope="all-hardware-checks")
    schema_path = root / "references/schemas/change-impact-plan.schema.json"
    try:
        validate_file(path, schema_path)
        impact = load_json(path)
    except (SchemaValidationError, OSError, json.JSONDecodeError) as exc:
        return None, make_check("change-impact-plan", "fail", error=str(exc))
    errors: list[str] = []
    if impact["skill_fingerprint"] != skill_fingerprint(root):
        errors.append("change-impact plan fingerprint is stale")
    if not impact["hardware_required"] or "L3" not in impact["required_levels"]:
        errors.append("change-impact plan does not require L3")
    if not impact["l3_cases"] and not impact["worker_checks"]:
        errors.append("change-impact plan selected no hardware checks")
    return impact, make_check(
        "change-impact-plan",
        "fail" if errors else "pass",
        errors=errors,
        path=str(path),
        l3_cases=impact["l3_cases"],
        worker_checks=impact["worker_checks"],
    )
...
def main() -> int:
    ...
    impact, impact_check = _load_impact(
        root, args.impact_plan.resolve() if args.impact_plan else None
    )
    ...
    selected_cases = set(impact["l3_cases"]) if impact else REQUIRED_OPERATOR_CLASSES
    selected_workers = (
        {item.replace("-", "_") for item in impact["worker_checks"]}
        if impact
        else {"submission", "failure_recovery"}
    )
    for case in suite["cases"]:
        if case["operator_class"] not in selected_cases:
            continue
