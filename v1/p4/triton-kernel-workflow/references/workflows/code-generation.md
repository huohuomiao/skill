...
For a focused Code Generation request, run the p4 preflight in `../contracts/cache-resume.md` with `mode=code-generation` before dispatch. Restore verified `reuse` stages and execute only stages marked `rerun`.
