...
2. `run` 时先 `start --stage <stage>`，然后调用对应 Skill；成功后执行 `complete`。
...
`complete` 会校验 `stage-sources.json` 声明的必需产物非空，自动收集本阶段已存在的可选审计产物，记录 SHA-256，并在适用时写入不可变缓存。额外产物可重复传 `--artifact <output-relative-path>`。因此 Triton 快速路径会连同 `original_code.py` 恢复，普通 Code Gen/Optimizer 也会保留中间 JSON、代码和报告。
...
检查点只有在阶段指纹与各产物哈希都一致时返回 `reusable=true`；输入或 Skill 源变化引发的阶段失效会清空其检查点。中断恢复只重开外层状态，保留仍有效的检查点。
