...
- `run_context_path`：已绑定的硬件与工具链上下文；
- `review_schema_path`：机器可读结果的 JSON Schema；
...
- `{同目录}/xxx_fix.py`
- `{同目录}/xxx_fix.md`
- `{同目录}/review_result.json`

迭代始终覆盖同一个 `xxx_fix.py` 并追加同一个报告，不产生 `_fix_1.py` 等编号文件。回复只
返回 `repaired`、`failed` 或 `infrastructure_error` 及三个路径。
...
同时读取 `run_context_path`，确认其中的 `execution_backend` 与 EnvConfig 一致。上下文缺失、
不一致或 Schema 非法都属于基础设施错误，不允许进入代码修复。
...
- 退出码 0 且精度断言通过：`repaired`；
- 连续两次出现完全相同的规范化错误签名：`failed`；
- 已执行五轮动态修改仍失败：`failed`；
- 出现基础设施错误：`infrastructure_error`。
...
`review_result.json` 必须通过 `review_schema_path`，并至少记录：

- `schema_version: 1`；
- `status`：`repaired`、`failed` 或 `infrastructure_error`；
- `validation_level: l3`；
- `execution_backend`、总执行次数和真实精度字段；
- `xxx_fix.py` 与 `xxx_fix.md` 的相对路径；
- 失败时的 `failure_reason`。

只有 `status=repaired` 且 `accuracy.pass=true` 才能被上游保存为 L3 Review 检查点。
