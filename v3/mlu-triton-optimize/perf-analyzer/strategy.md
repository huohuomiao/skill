...
调用方必须传入 `{output_dir}/Optimizer/optimization_state.json`。准备提交 Worker 前先执行：

```bash
python .claude/skills/mlu-triton-optimize/scripts/optimization_control.py check \
    --state <optimization_state.json绝对路径> --phase advanced
python .claude/skills/mlu-triton-optimize/scripts/optimization_control.py record \
    --state <optimization_state.json绝对路径> \
    --event worker --phase advanced --strategy perf-analyzer
```

预算检查退出码为 `3` 时不得运行 cnperf 或提交 Worker。
