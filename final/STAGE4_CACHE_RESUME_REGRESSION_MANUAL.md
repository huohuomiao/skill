# 历史基线说明：v4 缓存、断点续跑和持续回归

> 本文保留用于说明 final 结构继承自 v4 的控制面。final 的权威结构、L1/L2/L3 门禁和分阶段缓存规则以 [STRUCTURE_COMPARISON.md](STRUCTURE_COMPARISON.md)、[HOW_TO_VALIDATE.md](HOW_TO_VALIDATE.md) 及 `mlu-triton-main/references/stage-sources.json` 为准。
...
| `share/contracts/*.schema.json` | final 运行、验证、缓存与回归 JSON 契约 |
