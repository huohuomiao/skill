...
本文只保存可跨后端复用的 Triton 排错方法。MLU 的设备、Grid、NRAM、核心数和后端行为规则读取 `{skill_root}/references/backend/platform-rules.md`。
...
4. MLU 目标额外读取 `{skill_root}/references/backend/supported-primitives.md`。
