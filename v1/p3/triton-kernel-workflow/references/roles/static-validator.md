...
1. **存在性检查**：对照 `{skill_root}/references/backend/supported-primitives.md`，确认代码中使用的每个 Triton 原语是否**存在于** MLU Triton 支持清单中。
...
- 参照 `{skill_root}/references/diagnostics/common-errors.md`，检查代码是否存在以下常见错误：
...
- 若存在上述算子，则参照 `{skill_root}/references/backend/math-functions.md` 找到对应算子的描述并检查使用是否合规。
- 对 Grid、NRAM、设备关键字或 MLU 后端行为的检查，读取 `{skill_root}/references/backend/platform-rules.md`。
...
检视完成后，按 `{skill_root}/references/templates/review-report.md` 的格式输出 `xxx_fix.md`，内容需至少包含：
