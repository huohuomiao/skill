...
v3 的验证分为离线门禁、优化控制器命令验证、实际产物检查和 MLU 集成回归。完整设计见 [STAGE2_OPTIMIZATION_MANUAL.md](STAGE2_OPTIMIZATION_MANUAL.md)。

## 1. 环境

使用 Python 3.9 或更高版本。离线验证只使用标准库，不需要 PyYAML、pytest 或 jsonschema。

所有命令从 v3 根目录执行，并使用 `-B` 避免生成 `__pycache__`：
...
先把整套 v3 部署到目标项目 `.claude/skills`，在同一个 `JOB_ID` 中检查环境：
