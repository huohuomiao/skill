...
**核心目标**：根据输入的需求文档，生成可在 MLU 设备上正确执行的 Triton kernel 代码，包含**方案设计、代码生成、测试生成与最终验证**几个关键阶段。

## v4 断点续跑契约

由 `mlu-triton-main` 调用时，外层 `kernel_gen` 必须已经处于 `running`，并传入 `{output_dir}/run_manifest.json`。本 Skill 不得直接修改外层阶段状态，但必须使用控制器保存和验证内部检查点：

```bash
python .claude/skills/mlu-triton-main/scripts/run_control.py checkpoint-status \
  --manifest <run_manifest> --stage kernel_gen --name <checkpoint>
python .claude/skills/mlu-triton-main/scripts/run_control.py checkpoint-save \
  --manifest <run_manifest> --stage kernel_gen --name <checkpoint> \
  --artifact <output-relative-artifact> [--artifact <another-artifact>]
```
...
| 检查点 | 成功后必须记录的产物 |
|---|---|
| `step1` | `KernelGen/step1_base_info.json`、`KernelGen/step1_io_shapes.json` |
| `step2` | `KernelGen/step2_block_mapping.json` |
| `step3` | `KernelGen/step3_axis_fusion.json` |
| `step4` | `KernelGen/step4_code_spec.json` |
| `step5` | `KernelGen/step5_kernel_code.py` |
| `step6` | `KernelGen/step6_test_code.py` |
| `step7` | `KernelGen/step6_test_code_fix.py` |

规则：

- 进入每个 Step 前先查同名检查点；仅 `reusable=true` 时跳过该 Step，并继续使用已验 SHA-256 的产物。
- Step 首次完成且原有结构/语义校验通过后立即保存检查点；不得为失败或空产物建检查点。
- `resume` 会保留同一阶段指纹下的检查点；输入、上下文或相关 Skill 源变化时控制器会清空本阶段及下游检查点。
- Triton 快速路径仍跳过 Step 1-4，不创建伪检查点；从实际执行的 Step 5 开始记录。
- 单独调用本 Skill、没有 Main 清单时，按原流程执行，不伪造 `run_manifest.json`。
