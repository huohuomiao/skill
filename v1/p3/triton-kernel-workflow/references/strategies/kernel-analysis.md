...
复杂场景的完整分析例子放在 `references/examples/optimization/` 目录下，按需加载：

| 文件 | 内容 |
|------|------|
| `{skill_root}/references/examples/optimization/kernel-axis-reducemax.md` | 2D `[M,N]` 沿 `N` 轴分块求 max，`M` 轴无分块参数、`N` 轴被 for loop 遍历规约的分析例子 |
| `{skill_root}/references/examples/optimization/kernel-axis-softmax.md` | `N/H/W` 布局下沿 `W` 轴 softmax，`N/H` 合用 `BLOCK_NH` 分块的分析例子 |
| `{skill_root}/references/examples/optimization/kernel-axis-flatten.md` | 多个逻辑轴的 block 摊平成一维索引后再还原为各轴块索引的分析例子 |
