...
| **3. 平台/设备不一致**  | 代码中保留 `cuda` 关键字或fall back为cpu      | MLU 环境无法识别设备，运行失败 | 全量替换 `cuda` -> `mlu`，替换 `cpu` -> `mlu`， |
...
| **7. input device错误** | input的device是cpu或者cuda，不是mlu           | mlu后段无法处理cpu的input      | 将input的device统一设置为device="mlu"           |
...
3. 平台关键字未转换（CUDA vs MLU）
   错误现象：直接复用针对 NVIDIA 环境的代码，保留了 cuda 关键字或特定的环境变量。

后果：在寒武纪 MLU 环境下，代码无法识别设备，或者在底层调用时触发驱动异常。

```python
# ❌ 错误示例
x = torch.randn(1024, device='cuda') # ❌ 错误：不应在 MLU 环境下使用 cuda
x = torch.randn(1024, device='cpu')  # ❌ 错误：不应 fallback 到 CPU
x = torch.randn(1024, device='mlu') # ✅ 正确做法
```
