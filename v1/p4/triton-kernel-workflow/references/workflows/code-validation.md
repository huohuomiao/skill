...
For a focused validation request, run the p4 preflight in `../contracts/cache-resume.md` with `mode=code-validation`. A cache hit is accepted only after snapshot verification; otherwise perform the unchanged validation and repair flow.
