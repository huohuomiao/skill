# 阶段 3：按需优化与全局预算说明书（v3_1）

## 1. 版本目标

v3_1 在 v3 的 P0 修复、L1/L2/L3 验证体系和按需路由实现之上，正式固化阶段 3 的三个目标：

1. 不再对每个算子固定运行全部优化策略。
2. 不再允许深度优化、重试、Subagent 或 Worker 调用无限增长。
3. 重试或中断恢复不得重新生成状态、清零已经消耗的全局预算。

核心流程是“模式选择 → 静态路由 → 预算控制 → 有证据的候选选择”。v3_1 进一步把首次初始化和恢复分成两个互斥入口：普通 `plan` 只允许创建新状态，`plan --resume` 只允许复用完全兼容的旧状态。

本阶段没有引入 v4 的跨运行缓存和持续回归，也没有合并 Code Gen Subagent；这些不属于 v3_1 的范围。
...
```bash
python -B mlu-triton-optimize/scripts/optimization_control.py plan \
  --input kernel.py --output-dir output/Optimizer --mode balanced
```
...
```bash
python -B mlu-triton-optimize/scripts/optimization_control.py plan \
  --input kernel.py --output-dir output/Optimizer --mode balanced --resume
```

恢复必须满足：

- `optimization_plan.json` 和 `optimization_state.json` 同时存在。
- plan 的版本、输入 SHA-256、模式、预算、OOB 选中列表和 advanced 路由与当前请求一致。
- state 的版本、模式、预算和 `plan_path` 与 plan 一致。

成功时脚本只返回 `"resumed": true`，不改写 state。普通 `plan` 遇到已有状态、只有一个状态文件或恢复不兼容时退出码为 `2`。不得删除状态后重新初始化；真正的新输入、模式或预算必须使用新的输出目录。
...
对每个用例记录 v3/v3_1 的精度、总耗时、Subagent 数、Worker 数和最终性能，并增加“恢复前后 usage 是否保持”的断言。

## 14. 部署与回滚

部署时将 v3_1 内容作为一套整体 Skill 安装，不能只替换 `mlu-triton-optimize/SKILL.md`，否则控制脚本、Schema 和 Main 模式契约不匹配。
...
3. 运行 L1/L2。
4. 运行代表性 MLU 用例。
5. 比较 v3/v3_1 指标并验证预算恢复后再切换默认版本。
...
- v3_1 尚未实现跨运行缓存和受影响阶段增量重跑。
- v3_1 尚未合并 Code Gen 的多个串行 Subagent。
