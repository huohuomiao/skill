...
确认必需产物存在且非空后，通过状态更新脚本记录需求提取阶段。下游只从 `run_manifest.json` 获取状态与产物路径，不重复读取本角色的完整报告：

```bash
python {skill_root}/scripts/state/update-run-manifest.py \
  --manifest {输出存储路径}/run_manifest.json \
  --stage requirement-extraction \
  --status completed \
  --artifact requirement={输出存储路径}/Extractor/requirement.md
```

当 `input_type` 为 `triton` 时，在同一命令中增加 `--artifact original_code={输出存储路径}/Extractor/original_code.py`。
