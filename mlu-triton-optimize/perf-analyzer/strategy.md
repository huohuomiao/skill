...
python .claude/skills/mlu-triton-main/subagents/scripts/submit_task_to_worker.py \
    --workdir <仓库根目录的绝对路径> \
    --command "bash .claude/skills/mlu-triton-optimize/perf-analyzer/scripts/analyzer.sh <output_dir> <output_dir>/run_once.py" \
    --task-type custom
...
```bash
bash .claude/skills/mlu-triton-optimize/perf-analyzer/scripts/analyzer.sh  {output_dir}  {output_dir}/run_once.py
