...
def load_schemas(results: Results) -> dict[str, dict[str, Any]]:
    ...
    results.check(
        set(SCHEMA_BY_ARTIFACT.values()).issubset(schemas),
        "all artifact schemas exist",
        f"expected {sorted(SCHEMA_BY_ARTIFACT.values())}",
    )
    return schemas
...
def check_source_tree() -> Results:
    ...
    results.check(
        "步骤 3 已执行且 `{output_dir}/Optimizer/triton_advanced_optimized.py` 存在" in optimize,
        "advanced optimization output has priority when present",
    )
    results.check(
        "步骤 3 未执行或深度优化产物缺失" in optimize,
        "OOB optimization is the explicit fallback",
    )
    results.check(
