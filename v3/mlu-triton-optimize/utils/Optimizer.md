...
调用方必须同时传入 `optimization_plan.json`、`optimization_state.json` 和 `optimization_phase`（`oob` 或 `advanced`）。开始改写前确认当前策略出现在计划允许列表中：OOB 必须为 `selected=true`，高级策略必须属于 `advanced.candidates`；否则原样输出并标记 `skipped_by_plan`。

准备提交 Worker 前，必须先执行预算检查；退出码 `3` 时禁止提交：

```bash
python .claude/skills/mlu-triton-optimize/scripts/optimization_control.py check \
    --state <optimization_state.json绝对路径> \
    --phase <oob|advanced>
```

允许执行时，在调用 `submit_task_to_worker.py` 之前记录 Worker 次数：

```bash
python .claude/skills/mlu-triton-optimize/scripts/optimization_control.py record \
    --state <optimization_state.json绝对路径> \
    --event worker --phase <oob|advanced> --strategy <策略名称>
```

不得通过复制、删除或重新初始化状态文件来绕过预算。
...
先读取优化计划并确认策略允许执行。目标平台为 MLU 时，在读取具体策略前再读取 `.claude/skills/share/mlu/references/platform-rules.md`。若策略名称为 `libdevice-opt`，策略文档固定为 `.claude/skills/share/mlu/optimize/libdevice-opt.md`；其他策略继续使用调用方传入的文档路径。
