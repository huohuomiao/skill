...
本地环境可用时直接执行；本地不可用时，通过 `.claude/skills/mlu-bangc-main/subagents/scripts/submit_task_to_worker.py` 在当前 Job 的 MLU590 Worker 上执行同一组检查。
...
|---|---|
| 调用方 | `output_dir` |
| 设备探测 | `.claude/skills/share/mlu/runtime/get_device_info.py` |
| BANG C smoke test | `.claude/skills/share/mlu/runtime/test_env_code.py` 与同目录 `bangc_vector_add.mlu` |
| Worker 兜底 | `.claude/skills/mlu-bangc-main/subagents/scripts/submit_task_to_worker.py` |
...
- Worker 失败、Agent-Service 不可达或 `JOB_ID` 缺失时，选择 `execution_backend=unavailable`，写入真实错误后停止依赖运行结果的工作流。
...
## Smoke test
- source: share/mlu/runtime/bangc_vector_add.mlu
...
- 使用 C++ 标准库的 `.mlu` 编译链接至少保留 `-lcnrt -lstdc++ -lm -lpthread`；若当前工具链不接受，记录真实错误，留待服务器校准，不静默删减。
...
| 场景 | 处理 |
|---|---|
| `cnmon` 不可用或无 MLU590 | 保留日志，尝试 Worker |
| `cncc`/头文件/库不可定位 | 保留日志，尝试 Worker |
| smoke 编译失败 | 保存完整编译命令和 stderr，尝试 Worker |
| smoke 运行或精度失败 | 保存 stdout/stderr/max_diff，尝试 Worker |
