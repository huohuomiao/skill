...
description: 按需优化 Cambricon MLU 上的完整 Triton 算子，根据正确性、均衡或极致性能模式选择适用策略，并用全局预算限制 Subagent、Worker、策略尝试和深度迭代。
---
...
- 已存在 `optimization_plan.json` / `optimization_state.json` 时禁止再次初始化；恢复必须保留累计用量、最佳候选、patience、停止原因和历史。
...
首次进入且两个状态文件都不存在时，创建 `{output_dir}/Optimizer`，运行：
...
若两个状态文件已经存在，说明是同一次优化运行的重试或恢复，必须改用：
...
`--resume` 只在 plan/state 的版本、输入 SHA-256、模式、预算限制和策略路由全部兼容时成功，并且不改写已有状态。只有一个状态文件或校验不兼容时必须停止并使用新的输出目录；不得删除或重建状态来获取新预算。
