...
- `plan` 只用于 plan/state 都不存在的首次初始化；任一文件已存在时，不带 `--resume` 必须退出码 `2`。
- `plan --resume` 要求两个文件同时存在，并校验版本、输入哈希、mode、limits、OOB 选中列表、advanced 路由和 state 的 plan 路径。
- 恢复成功只返回 `"resumed": true`，不得改写 `optimization_state.json`。
- 只有一个文件或任一字段不兼容时退出码 `2`；调用方应使用新的输出目录，禁止删除状态后重新获取预算。
