...
def check_memory_status(devices):
    ...
    for dev in devices:
        if dev['memory_used'] > 0:
            ...
            print(f"\n[WARN] Card {dev['card_id']} 上有内存被占用 ({dev['memory_used']} MiB)")
...
def main():
    ...
    if cnmon_output is None:
        print("[FAIL] cnmon 命令不可用或执行失败")
        return False

    print("[PASS] cnmon 命令执行成功")

    # 解析设备信息
    devices = parse_device_info(cnmon_output)
    print(f"[PASS] 检测到 {len(devices)} 个 MLU 设备")

    if not devices:
        print("[FAIL] cnmon 未返回可解析的 MLU 设备")
        return False
