...
分块分析与改写属于通用 Triton 逻辑；MLU NRAM 和运行时约束读取 `{skill_root}/references/backend/platform-rules.md`。
...
参考 `{skill_root}/references/strategies/kernel-analysis.md`，从 triton kernel 中提取轴信息。

### 步骤2：`block_size` 全覆盖

查看从步骤1获取的轴信息中，若存在 `block_size` 为 `null` 的轴，则赋予该轴 `block_size`，并且使其通过 `tl.arange(0, BLOCK_SIZE)` 方式参与构建该维度上的张量。

详细参考 `{skill_root}/references/examples/optimization/parallel-tiling-template.md`。
