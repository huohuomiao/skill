...
def load_schemas(results: Results) -> dict[str, dict[str, Any]]:
    ...
    results.check(
        (
            ...
            | set(V4_SCHEMA_BY_ARTIFACT.values())
        ).issubset(schemas),
        "all artifact schemas exist",
        f"expected {sorted(set(SCHEMA_BY_ARTIFACT.values()) | set(OPTIMIZER_SCHEMA_BY_ARTIFACT.values()) | set(V4_SCHEMA_BY_ARTIFACT.values()))}",
    )
    return schemas
    ...
    with tempfile.TemporaryDirectory(prefix="mlu-triton-v4-") as temporary:
        ...
        results.check(
            quiet_call(
                ...
                argparse.Namespace(manifest=manifest_one, stage="env_config", artifact=[]),
            )
            == 0,
        ...
        results.check(
            quiet_call(
                run.command_complete,
                argparse.Namespace(manifest=manifest_one, stage="extractor", artifact=[]),
            )
            == 0,
        ...
        manifest = run.load_manifest(manifest_one)
        checkpoint_ok, checkpoint_reason = run.checkpoint_valid(manifest, "kernel_gen", "step1")
        ...
        results.check(
            quiet_call(
                run.command_complete,
                argparse.Namespace(manifest=manifest_one, stage="kernel_gen", artifact=[]),
            )
            == 0,
        ...
        quiet_call(
            run.command_complete,
            argparse.Namespace(manifest=manifest_two, stage="env_config", artifact=[]),
        )
        quiet_call(
            run.command_bind_context,
        ...
        quiet_call(
            run.command_complete,
            argparse.Namespace(manifest=manifest_three, stage="env_config", artifact=[]),
        )
        quiet_call(
            run.command_bind_context,
        ...
        quiet_call(
            run.command_complete,
            argparse.Namespace(manifest=manifest_four, stage="env_config", artifact=[]),
        )
        quiet_call(
            run.command_bind_context,
        ...
        quiet_call(
            run.command_complete,
            argparse.Namespace(manifest=manifest_four, stage="finalize", artifact=[]),
        )
        correctness_manifest = run.load_manifest(manifest_four)
