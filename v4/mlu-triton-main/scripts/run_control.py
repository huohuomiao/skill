...
def load_stage_config() -> dict[str, Any]:
    ...
    if config.get("version") != 1 or not isinstance(stages, list) or not stages:
...
def cache_metadata_valid(entry: Path, stage_name: str, fingerprint: str) -> tuple[bool, str | None]:
...
def next_action(manifest: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    for stage in stage_configs(config):
        ...
        if stage.get("cacheable"):
            ...
            valid, cache_reason = cache_metadata_valid(entry, name, row["fingerprint"])
            if valid:
...
def checkpoint_valid(manifest: dict[str, Any], stage_name: str, checkpoint_name: str) -> tuple[bool, str | None]:
...
def command_checkpoint_status(args: argparse.Namespace) -> int:
    ...
    refresh_manifest(manifest, config)
    require_stage(config, args.stage)
    valid, reason = checkpoint_valid(manifest, args.stage, args.name)
...
def populate_cache(manifest: dict[str, Any], stage: dict[str, Any], records: list[dict[str, Any]]) -> str | None:
    ...
    entry = cache_entry(manifest, name)
    if entry.exists():
        valid, reason = cache_metadata_valid(entry, name, fingerprint)
        if not valid:
    ...
    for record in records:
        ...
        destination = ensure_within(entry / "artifacts" / relative, entry, "cached artifact")
...
def command_complete(args: argparse.Namespace) -> int:
    ...
    cache_key = populate_cache(manifest, stage, records)
    row["status"] = "complete"
    row["reason"] = None
...
def command_restore(args: argparse.Namespace) -> int:
    ...
    valid, reason = cache_metadata_valid(entry, args.stage, row["fingerprint"])
    if not valid:
