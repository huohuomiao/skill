...
> **注**：`atomic_xxx` 包括 `atomic_add`, `cas`, `xchg`, `max`, `min`, `and`, `or`, `xor`。
...
| `argmax`, `argmin`, `max`, `min`, `sum` | **Yes** | 归约操作 (Reductions) |
...
4.  **类型提升规则**：
    MLU Triton 遵循 **C 语言类型提升逻辑**，这与官方 PyTorch 的提升规则存在差异，在编写混合类型计算时需特别注意。